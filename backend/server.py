...
SYSTEM_PROMPT = (
    ...
    "- python: has a live preview with REAL execution (Pyodide, runs in-browser) — write code that prints "
    "clear output so the result is visible, and it can install pip packages automatically.\n"
    "- node / other: provide the real files (e.g. /server.js). These have no live preview but the user will "
    "read the code.\n"
